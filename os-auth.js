...
function token(){
  ...
  if(!held() || !ls(K.refresh)) return new Promise(function(ok,no){ waiters.push({ok:ok,no:no}); });
...
function paintStyle(){
  ...
  '#osHomeBar{display:flex;align-items:center;gap:10px;padding:calc(6px + env(safe-area-inset-top)) 12px 6px;'+
  ...
  '#osHomeBar a{display:inline-flex;align-items:center;gap:6px;color:#fff;text-decoration:none;'+
    'background:rgba(255,255,255,.10);border:1px solid rgba(201,162,39,.45);border-radius:20px;padding:7px 13px}'+
  ...
  '#osGate .os-dots{display:flex;justify-content:center;gap:10px;height:14px;margin-bottom:8px}'+
...
function dots(){
  var box = gateEl.querySelector('.os-dots'), h = '', n = Math.max(6, typed.length);
...
function homeBar(){
  if(document.getElementById('osHomeBar')) return;
  paintStyle();
...
window.OSAuth = {
  ...
  ask: showGate,
  token: token,
  signOut: function(){ lsDel(K.refresh); lsDel(K.until); idToken = null; expiresAt = 0; },
